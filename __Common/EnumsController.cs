...
public class EnumsController : ControllerBase
{
    ...
    public IResult List()
    {
        var enumsList = _GetAppTypes()
            .Where(type => type.IsEnum && type.Namespace == AppConstants.EnumsNamespace)
            .Select(type =>
                {
    ...
    public IResult FindMany(string enumNames)
    {
        ...
        var enumsList = _GetAppTypes()
            .Where(type => type.Namespace == AppConstants.EnumsNamespace && type.IsEnum && enumNamesList.Contains(type.Name))
            .Select(type =>
                {
    ...
    {
        var enumType = Type.GetType($"{AppConstants.EnumsNamespace}.{enumName}");
        if(enumType is null)
